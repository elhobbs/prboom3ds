# 1 "c:/devl/prboom3ds/src/r_segs.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/r_segs.c"
# 36 "c:/devl/prboom3ds/src/r_segs.c"
...
# 37 "c:/devl/prboom3ds/src/r_segs.c" 2
...
# 38 "c:/devl/prboom3ds/src/r_segs.c" 2
...
# 39 "c:/devl/prboom3ds/src/r_segs.c" 2
# 1 "c:/devl/prboom3ds/src/r_segs.h" 1
# 41 "c:/devl/prboom3ds/src/r_segs.h"
void R_RenderMaskedSegRange(drawseg_t *ds, int x1, int x2);
void R_StoreWallRange(const int start, const int stop);
# 40 "c:/devl/prboom3ds/src/r_segs.c" 2
...
# 41 "c:/devl/prboom3ds/src/r_segs.c" 2
...
# 42 "c:/devl/prboom3ds/src/r_segs.c" 2
...
# 44 "c:/devl/prboom3ds/src/r_segs.c" 2
...
void V_FreeScreens();
# 45 "c:/devl/prboom3ds/src/r_segs.c" 2
...
# 46 "c:/devl/prboom3ds/src/r_segs.c" 2
...
# 103 "c:/devl/prboom3ds/src/r_segs.c"
static fixed_t R_ScaleFromGlobalAngle(angle_t visangle)
{
  int anglea = 0x40000000 + (visangle-viewangle);
  int angleb = 0x40000000 + (visangle-rw_normalangle);
  int den = FixedMul(rw_distance, finesine[anglea>>19]);

  fixed_t num = FixedMul(projectiony, finesine[angleb>>19]);
  return den > num>>16 ? (num = FixedDiv(num, den)) > 64*(1<<16) ?
    64*(1<<16) : num < 256 ? 256 : num : 64*(1<<16);
}
...
void R_RenderMaskedSegRange(drawseg_t *ds, int x1, int x2)
{
  ...
  rw_lightlevel = R_FakeFlat(frontsector, &tempsec, ((void *)0), ((void *)0), 0) ->lightlevel;
  ...
  for (dcvars.x = x1 ; dcvars.x <= x2 ; dcvars.x++, spryscale += rw_scalestep)
    if (maskedtexturecol[dcvars.x] != 2147483647)
      {

        angle = (ds->rw_centerangle + xtoviewangle[dcvars.x]) >> 19;
...
# 233 "c:/devl/prboom3ds/src/r_segs.c"
        ...
        maskedtexturecol[dcvars.x] = 2147483647;
      }
...
# 263 "c:/devl/prboom3ds/src/r_segs.c"
  ...
  for ( ; rw_x < rw_stopx ; rw_x++)
    {
      ...
      int yh = bottomfrac>>12;
      int yl = (topfrac+(1<<12)-1)>>12;
      ...
      if (segtextured)
        {
          ...
          angle_t angle =(rw_centerangle+xtoviewangle[rw_x])>>19;

          texturecolumn = rw_offset-FixedMul(finetangent[angle],rw_distance);
          if (drawvars.filterwall == RDRAW_FILTER_LINEAR)
            texturecolumn -= ((1<<16)>>1);
...
static fixed_t R_PointToDist(fixed_t x, fixed_t y)
{
  ...
  return FixedDiv(dx, finesine[(tantoangle[FixedDiv(dy,dx) >> (16 -11)]
                                + 0x40000000) >> 19]);
}
...
void R_StoreWallRange(const int start, const int stop)
{
  ...
  if(curline->miniseg == 0)
    curline->linedef->flags |= 256;
# 509 "c:/devl/prboom3ds/src/r_segs.c"
      ...
      for (ds = drawsegs; ds < ds_p; ds++)
        {
          ...
          if (ds->maskedtexturecol + ds->x1 >= oldopenings && ds->maskedtexturecol + ds->x1 <= oldlast) ds->maskedtexturecol = ds->maskedtexturecol - oldopenings + openings;;
          if (ds->sprtopclip + ds->x1 >= oldopenings && ds->sprtopclip + ds->x1 <= oldlast) ds->sprtopclip = ds->sprtopclip - oldopenings + openings;;
          if (ds->sprbottomclip + ds->x1 >= oldopenings && ds->sprbottomclip + ds->x1 <= oldlast) ds->sprbottomclip = ds->sprbottomclip - oldopenings + openings;;
        }
  ...
  if (!backsector)
    {
      ...
      midtexheight = (linedef->r_flags & RF_MID_TILE) ? 0 : textureheight[midtexture] >> 16;
...
# 626 "c:/devl/prboom3ds/src/r_segs.c"
  ds_p->silhouette = 3;
  ds_p->sprbottomclip = negonearray;
  ds_p->bsilheight = 2147483647;
  ds_p->sprtopclip = screenheightarray;
  ...
  if (frontsector->ceilingheight < backsector->ceilingheight)
    ...
    toptexheight = (linedef->r_flags & RF_TOP_TILE) ? 0 : textureheight[toptexture] >> 16;
          rw_toptexturemid = linedef->flags & 8 ? worldtop :
    ...
    bottomtexheight = (linedef->r_flags & RF_BOT_TILE) ? 0 : textureheight[bottomtexture] >> 16;
          rw_bottomtexturemid = linedef->flags & 16 ? worldtop :
            worldlow;
  ...
  if (segtextured)
    {
      rw_offset = FixedMul (hyp, -finesine[offsetangle >>19]);

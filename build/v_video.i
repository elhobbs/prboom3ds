# 1 "c:/devl/prboom3ds/src/v_video.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/v_video.c"
# 38 "c:/devl/prboom3ds/src/v_video.c"
...
# 39 "c:/devl/prboom3ds/src/v_video.c" 2
# 1 "c:/devl/prboom3ds/src/r_main.h" 1
# 37 "c:/devl/prboom3ds/src/r_main.h"
...
# 38 "c:/devl/prboom3ds/src/r_main.h" 2
...
# 40 "c:/devl/prboom3ds/src/v_video.c" 2
...
# 41 "c:/devl/prboom3ds/src/v_video.c" 2
...
# 42 "c:/devl/prboom3ds/src/v_video.c" 2
...
# 43 "c:/devl/prboom3ds/src/v_video.c" 2
...
void V_FreeScreens();
# 44 "c:/devl/prboom3ds/src/v_video.c" 2
...
# 45 "c:/devl/prboom3ds/src/v_video.c" 2
...
# 46 "c:/devl/prboom3ds/src/v_video.c" 2
...
# 47 "c:/devl/prboom3ds/src/v_video.c" 2


screeninfo_t screens[6];


const byte *colrngs[CR_LIMIT];

int usegamma;
# 71 "c:/devl/prboom3ds/src/v_video.c"
typedef struct {
  const char *name;
  const byte **map;
} crdef_t;
...
void V_InitColorTranslation(void)
{
  ...
  for (p=crdefs; p->name; p++)
    *p->map = W_CacheLumpNum (W_GetNumForName(p->name));
}
# 112 "c:/devl/prboom3ds/src/v_video.c"
...
# 140 "c:/devl/prboom3ds/src/v_video.c"
...
# 159 "c:/devl/prboom3ds/src/v_video.c"
static inline byte GETCOL8(byte col)
  { return col; }
static inline short GETCOL15(byte col)
  { return V_Palette15[ (col)*64 + ((64 -1)) ]; }
static inline short GETCOL16(byte col)
  { return V_Palette16[ (col)*64 + ((64 -1)) ]; }
static inline int GETCOL32(byte col)
  { return V_Palette32[ (col)*64 + ((64 -1)) ]; }
# 188 "c:/devl/prboom3ds/src/v_video.c"
static void FUNC_V_DrawBackground(const char* flatname, int scrn)
{
  ...
  {
  case VID_MODE8:
    { const int width = (64 * SCREENWIDTH) / 320; int height = (64 * SCREENHEIGHT) / 200; fixed_t dx = (320 << 16) / SCREENWIDTH; byte *dest = (byte *)screens[scrn].data; while (height--) { const byte *const src_row = src + 64*((height*200)/SCREENHEIGHT); byte *const dst_row = dest + screens[scrn].byte_pitch*height; int x; fixed_t tx; for (x=0, tx=0; x<width; x++, tx += dx) { byte col = src_row[tx >> 16]; dst_row[x] = GETCOL8(col); } } };
    break;
  case VID_MODE15:
    { const int width = (64 * SCREENWIDTH) / 320; int height = (64 * SCREENHEIGHT) / 200; fixed_t dx = (320 << 16) / SCREENWIDTH; short *dest = (short *)screens[scrn].data; while (height--) { const byte *const src_row = src + 64*((height*200)/SCREENHEIGHT); short *const dst_row = dest + screens[scrn].short_pitch*height; int x; fixed_t tx; for (x=0, tx=0; x<width; x++, tx += dx) { byte col = src_row[tx >> 16]; dst_row[x] = GETCOL15(col); } } };
    break;
  case VID_MODE16:
    { const int width = (64 * SCREENWIDTH) / 320; int height = (64 * SCREENHEIGHT) / 200; fixed_t dx = (320 << 16) / SCREENWIDTH; short *dest = (short *)screens[scrn].data; while (height--) { const byte *const src_row = src + 64*((height*200)/SCREENHEIGHT); short *const dst_row = dest + screens[scrn].short_pitch*height; int x; fixed_t tx; for (x=0, tx=0; x<width; x++, tx += dx) { byte col = src_row[tx >> 16]; dst_row[x] = GETCOL16(col); } } };
    break;
  case VID_MODE32:
    { const int width = (64 * SCREENWIDTH) / 320; int height = (64 * SCREENHEIGHT) / 200; fixed_t dx = (320 << 16) / SCREENWIDTH; int *dest = (int *)screens[scrn].data; while (height--) { const byte *const src_row = src + 64*((height*200)/SCREENHEIGHT); int *const dst_row = dest + screens[scrn].int_pitch*height; int x; fixed_t tx; for (x=0, tx=0; x<width; x++, tx += dx) { byte col = src_row[tx >> 16]; dst_row[x] = GETCOL32(col); } } };
    break;
  default: return;
  }
...
# 232 "c:/devl/prboom3ds/src/v_video.c"
void V_Init (void)
{
  int i;
  ...
  for (i = 0; i<6; i++) {
    screens[i].data = ((void *)0);
    screens[i].not_on_heap = 0;
    screens[i].width = 0;
    screens[i].height = 0;
...
# 260 "c:/devl/prboom3ds/src/v_video.c"
        ...
        int cm, enum patch_translation_e flags)
{
  ...
  if (V_GetMode() == VID_MODE8 && !(flags & VPT_STRETCH)) {
    ...
    left = ( x * DX ) >> 16;
    top = ( y * DY ) >> 16;
    right = ( (x + patch->width) * DX ) >> 16;
    bottom = ( (y + patch->height) * DY ) >> 16;

    dcvars.texheight = patch->height;
    dcvars.iscale = DYI;
    dcvars.drawingmasked = ((patch->width)>(patch->height)?(patch->width):(patch->height)) > 8;
    ...
    for (dcvars.x=left; dcvars.x<right; dcvars.x++, col+=DXI) {
      ...
      dcvars.texu = ((flags & VPT_FLIP) ? ((patch->width<<16)-col) : col) % (patch->width<<16);
      ...
      for (i=0; i<column->numPosts; i++) {
        ...
        dcvars.yl = (((y + post->topdelta) * DY)>>16);
        dcvars.yh = (((y + post->topdelta + post->length) * DY - ((1<<16)>>1))>>16);
...
unsigned short *V_Palette15 = ((void *)0);
unsigned short *V_Palette16 = ((void *)0);
unsigned int *V_Palette32 = ((void *)0);
static unsigned short *Palettes15 = ((void *)0);
static unsigned short *Palettes16 = ((void *)0);
static unsigned int *Palettes32 = ((void *)0);
...
void V_UpdateTrueColorPalette(video_mode_t mode) {
  ...
  if (usegammaOnLastPaletteGeneration != usegamma) {
    if (Palettes15) Z_Free(Palettes15);
    if (Palettes16) Z_Free(Palettes16);
    if (Palettes32) Z_Free(Palettes32);
    Palettes15 = ((void *)0);
    Palettes16 = ((void *)0);
    Palettes32 = ((void *)0);
  ...
  if (mode == VID_MODE32) {
    if (!Palettes32) {
      ...
      Palettes32 = (int*)Z_Malloc(numPals*256*sizeof(int)*64,PU_STATIC,0);
      for (p=0; p<numPals; p++) {
        for (i=0; i<256; i++) {
          ...
          for (w=0; w<64; w++) {
            t = (float)(w)/(float)(64 -1);
            nr = (int)(r*t+roundUpR);
            ng = (int)(g*t+roundUpG);
            nb = (int)(b*t+roundUpB);
            Palettes32[((p*256+i)*64)+w] = (
              (nr<<16) | (ng<<8) | nb
            );
          }
        }
    ...
    V_Palette32 = Palettes32 + paletteNum*256*64;
  }
  else if (mode == VID_MODE16) {
    if (!Palettes16) {

      Palettes16 = (short*)Z_Malloc(numPals*256*sizeof(short)*64,PU_STATIC,0);
      for (p=0; p<numPals; p++) {
        for (i=0; i<256; i++) {
          r = gtable[pal[(256*p+i)*3+0]];
          g = gtable[pal[(256*p+i)*3+1]];
    ...
    V_Palette16 = Palettes16 + paletteNum*256*64;
  }
  else if (mode == VID_MODE15) {
    if (!Palettes15) {

      Palettes15 = (short*)Z_Malloc(numPals*256*sizeof(short)*64,PU_STATIC,0);
      for (p=0; p<numPals; p++) {
        for (i=0; i<256; i++) {
          r = gtable[pal[(256*p+i)*3+0]];
          g = gtable[pal[(256*p+i)*3+1]];
    ...
    V_Palette15 = Palettes15 + paletteNum*256*64;
  }
...
void V_SetPalette(int pal)
{
  ...
  if (V_GetMode() == VID_MODEGL) {
    ...
    if (V_GetMode() == VID_MODE15 || V_GetMode() == VID_MODE16 || V_GetMode() == VID_MODE32) {
      ...
      if ((W_CheckNumForName)("PLAYPAL", ns_global) >= 0) {
...
static void V_FillRect15(int scrn, int x, int y, int width, int height, byte colour)
{
  ...
  int w;
  short c = V_Palette15[ (colour)*64 + ((64 -1)) ];
  while (height--) {
    for (w=0; w<width; w++) {
      dest[w] = c;
    }
...
static void V_FillRect16(int scrn, int x, int y, int width, int height, byte colour)
{
  unsigned short* dest = (unsigned short *)screens[scrn].data + x + y*screens[scrn].short_pitch;
  int w;
  short c = V_Palette16[ (colour)*64 + ((64 -1)) ];
  while (height--) {
    for (w=0; w<width; w++) {
      dest[w] = c;
    }
...
static void V_FillRect32(int scrn, int x, int y, int width, int height, byte colour)
{
  ...
  int w;
  int c = V_Palette32[ (colour)*64 + ((64 -1)) ];
  while (height--) {
    for (w=0; w<width; w++) {
      dest[w] = c;
    }
...
# 779 "c:/devl/prboom3ds/src/v_video.c"
...
void V_InitMode(video_mode_t mode) {
  ...
  if (mode == VID_MODEGL)
    mode = VID_MODE8;
# 816 "c:/devl/prboom3ds/src/v_video.c"
  switch (mode) {
    case VID_MODE8:
    ...
    case VID_MODE32:
      ...
      break;
# 883 "c:/devl/prboom3ds/src/v_video.c"
  }
  R_FilterInit();
}
...
void V_AllocScreen(screeninfo_t *scrn) {
  if (!scrn->not_on_heap)
    if ((scrn->byte_pitch * scrn->height) > 0)
      scrn->data = Z_Malloc(scrn->byte_pitch*scrn->height,PU_STATIC,0);
}
...
static void V_PlotPixel15(int scrn, int x, int y, byte color) {
  ((unsigned short *)screens[scrn].data)[x+screens[scrn].short_pitch*y] = V_Palette15[ (color)*64 + ((64 -1)) ];
}

static void V_PlotPixel16(int scrn, int x, int y, byte color) {
  ((unsigned short *)screens[scrn].data)[x+screens[scrn].short_pitch*y] = V_Palette16[ (color)*64 + ((64 -1)) ];
}

static void V_PlotPixel32(int scrn, int x, int y, byte color) {
  ((unsigned int *)screens[scrn].data)[x+screens[scrn].int_pitch*y] = V_Palette32[ (color)*64 + ((64 -1)) ];
}
# 991 "c:/devl/prboom3ds/src/v_video.c"
static void WRAP_V_DrawLine(fline_t* fl, int color)
{
  register int x;
  register int y;
  ...
  register int ay;
  register int d;
# 1023 "c:/devl/prboom3ds/src/v_video.c"
  dx = fl->b.x - fl->a.x;
  ax = 2 * (dx<0 ? -dx : dx);
  sx = dx<0 ? -1 : 1;

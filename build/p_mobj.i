# 1 "c:/devl/prboom3ds/src/p_mobj.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/p_mobj.c"
# 34 "c:/devl/prboom3ds/src/p_mobj.c"
...
# 35 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 36 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 37 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 38 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 39 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 40 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 41 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 42 "c:/devl/prboom3ds/src/p_mobj.c" 2
# 1 "c:/devl/prboom3ds/src/st_stuff.h" 1
# 40 "c:/devl/prboom3ds/src/st_stuff.h"
...
# 41 "c:/devl/prboom3ds/src/st_stuff.h" 2
...
# 43 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 44 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
extern int idmusnum;
# 45 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 47 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 48 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 49 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
# 50 "c:/devl/prboom3ds/src/p_mobj.c" 2
...
void P_ExplodeMissile (mobj_t* mo)
{
  ...
  mo->flags &= ~(uint_64_t)(0x0000000000010000);

  if (mo->info->deathsound)
    S_StartSound (mo, mo->info->deathsound);
}
# 138 "c:/devl/prboom3ds/src/p_mobj.c"
static void P_XYMovement (mobj_t* mo)
{
  ...
  fixed_t oldx,oldy;
# 154 "c:/devl/prboom3ds/src/p_mobj.c"
  if (!(mo->momx | mo->momy))
    {
    if (mo->flags & (uint_64_t)(0x0000000001000000))
      {



      mo->flags &= ~(uint_64_t)(0x0000000001000000);
      mo->momz = 0;
  ...
  (!comp[comp_moveblock]
   && (xmove < -(30*(1<<16))/2 || ymove < -(30*(1<<16))/2)))
      {
      ptryx = mo->x + xmove/2;
      ptryy = mo->y + ymove/2;
      xmove >>= 1;
    ...
    if (!(mo->flags & (uint_64_t)(0x0000000000010000)) &&
        (compatibility_level>=mbf_compatibility) &&
        (mo->flags & (uint_64_t)0x0000000200000000ll ||
         (!player && blockline &&
    variable_friction && mo->z <= mo->floorz &&
    P_GetFriction(mo, ((void *)0)) > 0xE800)))
      {
        if (blockline)
    {
      fixed_t r = ((blockline->dx >> 16) * mo->momx +
             (blockline->dy >> 16) * mo->momy) /
        ((blockline->dx >> 16)*(blockline->dx >> 16)+
         (blockline->dy >> 16)*(blockline->dy >> 16));
      ...
      if (!(mo->flags & (uint_64_t)(0x0000000000000200)))
        {
          mo->momx = (mo->momx + x)/2;
          mo->momy = (mo->momy + y)/2;
        }
...
# 297 "c:/devl/prboom3ds/src/p_mobj.c"
  if (mo->flags & ((uint_64_t)(0x0000000000010000) | (uint_64_t)(0x0000000001000000)) || mo->z > mo->floorz)
    return;
  ...
  if (((mo->flags & (uint_64_t)0x0000000200000000ll && mo->z > mo->dropoffz) ||
       mo->flags & (uint_64_t)(0x0000000000100000) || mo->intflags & MIF_FALLING) &&
      (mo->momx > (1<<16)/4 || mo->momx < -(1<<16)/4 ||
       mo->momy > (1<<16)/4 || mo->momy < -(1<<16)/4) &&
  ...
  if (mo->momx > -((1<<16)/16) && mo->momx < ((1<<16)/16) &&
      mo->momy > -((1<<16)/16) && mo->momy < ((1<<16)/16) &&
  ...
  player->momx = player->momy = 0;
    ...
    {
# 359 "c:/devl/prboom3ds/src/p_mobj.c"
      ...
      fixed_t friction = P_GetFriction(mo, ((void *)0));
      ...
      if (player && player->mo == mo)
  {
    player->momx = FixedMul(player->momx, 0xE800);
...
static void P_ZMovement (mobj_t* mo)
{
  ...
  if (mo->flags & (uint_64_t)0x0000000200000000ll && mo->momz) {
    mo->z += mo->momz;
    if (mo->z <= mo->floorz) {
      mo->z = mo->floorz;
      if (mo->momz < 0) {
        mo->momz = -mo->momz;
  if (!(mo->flags & (uint_64_t)(0x0000000000000200))) {
    mo->momz = mo->flags & (uint_64_t)(0x0000000000004000) ?
      mo->flags & (uint_64_t)(0x0000000000000400) ?
      FixedMul(mo->momz, (fixed_t)((1<<16)*.85)) :
      FixedMul(mo->momz, (fixed_t)((1<<16)*.70)) :
      FixedMul(mo->momz, (fixed_t)((1<<16)*.45)) ;


    if (D_abs(mo->momz) <= mo->info->mass*((1<<16)*4/256))
      mo->momz = 0;
  }


  if (mo->flags & (uint_64_t)0x0000000100000000ll && mo->intflags & MIF_ARMED
      && mo->health > 0)
    P_DamageMobj(mo, ((void *)0), ((void *)0), mo->health);
  else if (mo->flags & (uint_64_t)(0x0000000000004000) && ((mo)->health > 0 && (mo)->info->seestate))
    goto floater;
  return;
      }
  ...
  if (mo->subsector->sector->ceilingpic != skyflatnum)
    mo->momz = -mo->momz;
  else if (mo->flags & (uint_64_t)(0x0000000000010000))
    P_RemoveMobj(mo);
  else if (mo->flags & (uint_64_t)(0x0000000000000200))
    mo->momz = -mo->momz;

  if (mo->flags & (uint_64_t)(0x0000000000004000) && ((mo)->health > 0 && (mo)->info->seestate))
    goto floater;
  ...
  return;
      ...
      if (!(mo->flags & (uint_64_t)(0x0000000000000200)))
        mo->momz -= mo->info->mass*((1<<16)/256);

      if (mo->flags & (uint_64_t)(0x0000000000004000) && ((mo)->health > 0 && (mo)->info->seestate)) goto floater;
      return;
    }
...
floater:
  if ((mo->flags & (uint_64_t)(0x0000000000004000)) && mo->target)



    if (!((mo->flags ^ (uint_64_t)(0x0000000000004000)) & ((uint_64_t)(0x0000000000004000) | (uint_64_t)(0x0000000001000000) | (uint_64_t)(0x0000000000200000))) &&
  mo->target)
      {
  fixed_t delta;
  if (P_AproxDistance(mo->x - mo->target->x, mo->y - mo->target->y) <
      D_abs(delta = mo->target->z + (mo->height>>1) - mo->z)*3)
    mo->z += delta < 0 ? -((1<<16)*4) : ((1<<16)*4);
      }
  ...
  if (mo->z <= mo->floorz)
    {
# 548 "c:/devl/prboom3ds/src/p_mobj.c"
    if (mo->flags & (uint_64_t)(0x0000000001000000) &&
 (!comp[comp_soul] ||
  ...
  if (mo->flags & (uint_64_t)0x0000000100000000ll && mo->intflags & MIF_ARMED && mo->health > 0)
    P_DamageMobj(mo, ((void *)0), ((void *)0), mo->health);
  else
    if (mo->player &&
        mo->player->mo == mo && mo->momz < -(1<<16)*8)
      {
 ...
 compatibility_level <= doom2_19_compatibility)
    ...
    if ( (mo->flags & (uint_64_t)(0x0000000000010000)) && !(mo->flags & (uint_64_t)(0x0000000000001000)) )
      {
      P_ExplodeMissile (mo);
      return;
      }
  ...
  if (mo->z + mo->height > mo->ceilingz)
    ...
    if (!comp[comp_soul] && mo->flags & (uint_64_t)(0x0000000001000000))
      mo->momz = -mo->momz;
...
# 662 "c:/devl/prboom3ds/src/p_mobj.c"
  ...
  if (mobj->info->flags & (uint_64_t)(0x0000000000000100))
    z = 2147483647;
  else
    z = (-2147483647 - 1);
  ...
  if (mthing->options & 8)
    mo->flags |= (uint_64_t)(0x0000000000000020);


  mo->flags = (mo->flags & ~(uint_64_t)0x0000000400000000ll) | (mobj->flags & (uint_64_t)0x0000000400000000ll);
...
void P_MobjThinker (mobj_t* mobj)
{
  ...
  if (mobj->momx | mobj->momy || mobj->flags & (uint_64_t)(0x0000000001000000))
    {
      P_XYMovement(mobj);
  ...
  return;
    ...
    if (!(mobj->momx | mobj->momy) && !((mobj)->health > 0 && (mobj)->info->seestate))
      {
  ...
  if (mobj->z > mobj->dropoffz &&
      !(mobj->flags & (uint_64_t)(0x0000000000000200)) &&
      !comp[comp_falloff])
    P_ApplyTorque(mobj);
  else
  ...
  if (mobj->tics != -1)
    ...
    if (! (mobj->flags & (uint_64_t)(0x0000000000400000)) )
      return;
...
mobj_t* P_SpawnMobj(fixed_t x,fixed_t y,fixed_t z,mobjtype_t type)
{
  ...
  if (!(compatibility_level>=mbf_compatibility))
    mobj->flags &= ~((uint_64_t)0x0000000200000000ll | (uint_64_t)0x0000000400000000ll | (uint_64_t)0x0000000100000000ll);
  else
    if (type == MT_PLAYER)
      mobj->flags |= (uint_64_t)0x0000000400000000ll;
  ...
  mobj->lastlook = P_Random (pr_lastlook) % 4;
  ...
  mobj->sprite = st->sprite;
  mobj->frame = st->frame;
  mobj->touching_sectorlist = ((void *)0);
  ...
  mobj->z = z == (-2147483647 - 1) ? mobj->floorz : z == 2147483647 ?
  ...
  mobj->friction = 0xE800;

  mobj->target = mobj->tracer = mobj->lastenemy = ((void *)0);
  P_AddThinker (&mobj->thinker);
  if (!((mobj->flags ^ (uint_64_t)(0x0000000000400000)) & ((uint_64_t)0x0000000400000000ll | (uint_64_t)(0x0000000000400000))))
    totallive++;
  return mobj;
}


static mapthing_t itemrespawnque[128];
static int itemrespawntime[128];
int iquehead;
int iquetail;
...
{
  if ((mobj->flags & (uint_64_t)(0x0000000000000001))
      && !(mobj->flags & (uint_64_t)(0x0000000000020000))
      && (mobj->type != MT_INV)
      && (mobj->type != MT_INS))
    {
  ...
  S_StopSound (mobj);
# 933 "c:/devl/prboom3ds/src/p_mobj.c"
  if ((compatibility_level >= lxdoom_1_compatibility) ||
      (!demorecording && !demoplayback)) {
    P_SetTarget(&mobj->target, ((void *)0));
    P_SetTarget(&mobj->tracer, ((void *)0));
    P_SetTarget(&mobj->lastenemy, ((void *)0));
  }


  P_RemoveThinker (&mobj->thinker);
}
# 953 "c:/devl/prboom3ds/src/p_mobj.c"
static __attribute__((pure)) int P_FindDoomedNum(unsigned type)
{
...
void P_RespawnSpecials (void)
{
  ...
  if (mobjinfo[i].flags & (uint_64_t)(0x0000000000000100))
    z = 2147483647;
  else
    z = (-2147483647 - 1);
...
# 1046 "c:/devl/prboom3ds/src/p_mobj.c"
...
void P_SpawnPlayer (int n, const mapthing_t* mthing)
{
  ...
  mobj->flags |= playernumtotrans[n]<<26;
...
# 1181 "c:/devl/prboom3ds/src/p_mobj.c"
  if ((compatibility_level < boom_compatibility_compatibility) ||
  ...
  if (mthing->type == 11)
    {
    if (!(!(compatibility_level<=boom_compatibility_compatibility) || deathmatch_p-deathmatchstarts < 10)) {
  return;
 } else {
    ...
    if (offset >= num_deathmatchstarts)
      ...
      deathmatchstarts = Z_Realloc(deathmatchstarts,num_deathmatchstarts * sizeof(*deathmatchstarts),PU_STATIC,0)
  ...
  if (!netgame && options & 16)
    return;



  if (netgame && deathmatch && options & 32)
    return;



  if (netgame && !deathmatch && options & 64)
    return;
  ...
  if (deathmatch && mobjinfo[i].flags & (uint_64_t)(0x0000000002000000))
    return;



  if (nomonsters && (i == MT_SKULL || (mobjinfo[i].flags & (uint_64_t)(0x0000000000400000))))
    return;
...
spawnit:
  ...
  if (!(mobj->flags & (uint_64_t)0x0000000400000000ll) &&
      options & 128 &&
      (compatibility_level>=mbf_compatibility))
    {
      mobj->flags |= (uint_64_t)0x0000000400000000ll;
  ...
  if (mobj->flags & (uint_64_t)(0x0000000000800000))
    totalitems++;

  mobj->angle = 0x20000000 * (mthing->angle/45);
  if (options & 8)
    mobj->flags |= (uint_64_t)(0x0000000000000020);
}
# 1359 "c:/devl/prboom3ds/src/p_mobj.c"
...
# 1411 "c:/devl/prboom3ds/src/p_mobj.c"
void P_CheckMissileSpawn (mobj_t* th)
{
  ...
  if (!(th->flags & (uint_64_t)(0x0000000000010000)) && (compatibility_level>=mbf_compatibility))
    return;
...
mobj_t* P_SpawnMissile(mobj_t* source,mobj_t* dest,mobjtype_t type)
{
  ...
  th = P_SpawnMobj (source->x,source->y,source->z + 4*8*(1<<16),type);
  ...
  if (dest->flags & (uint_64_t)(0x0000000000040000))
    {
...
void P_SpawnPlayerMissile(mobj_t* source,mobjtype_t type)
{
  ...
  angle_t an = source->angle;
...

    {
      ...
      uint_64_t mask = (compatibility_level>=mbf_compatibility) ? (uint_64_t)0x0000000400000000ll : 0;

      do
  {
    slope = P_AimLineAttack(source, an, 16*64*(1<<16), mask);
    if (!linetarget)
      slope = P_AimLineAttack(source, an += 1<<26, 16*64*(1<<16), mask);
    if (!linetarget)
      slope = P_AimLineAttack(source, an -= 2<<26, 16*64*(1<<16), mask);
    if (!linetarget)

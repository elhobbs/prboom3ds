# 1 "c:/devl/prboom3ds/src/p_inter.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/p_inter.c"
# 34 "c:/devl/prboom3ds/src/p_inter.c"
...
# 35 "c:/devl/prboom3ds/src/p_inter.c" 2
...
# 36 "c:/devl/prboom3ds/src/p_inter.c" 2
...
# 37 "c:/devl/prboom3ds/src/p_inter.c" 2
...
# 38 "c:/devl/prboom3ds/src/p_inter.c" 2
...
# 39 "c:/devl/prboom3ds/src/p_inter.c" 2
...
extern int idmusnum;
# 40 "c:/devl/prboom3ds/src/p_inter.c" 2
...
# 41 "c:/devl/prboom3ds/src/p_inter.c" 2
...
# 42 "c:/devl/prboom3ds/src/p_inter.c" 2
...
# 43 "c:/devl/prboom3ds/src/p_inter.c" 2
...
# 44 "c:/devl/prboom3ds/src/p_inter.c" 2
...
# 46 "c:/devl/prboom3ds/src/p_inter.c" 2
...
void A_RandomJump();
void A_LineEffect();
# 47 "c:/devl/prboom3ds/src/p_inter.c" 2
# 60 "c:/devl/prboom3ds/src/p_inter.c"
...
# 96 "c:/devl/prboom3ds/src/p_inter.c"
  ...
  if (netgame && deathmatch!=2 && !dropped)
    {
      ...
      if (!comp[comp_sound] || player == &players[displayplayer])
        S_StartSound (player->mo, sfx_wpnup|(0x8000));
      return 0;
    }
  ...
  switch (power)
    {
      case pw_invisibility:
        player->mo->flags |= (uint_64_t)(0x0000000000040000);
        break;
      case pw_allmap:
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher)
{
  ...
  switch (special->sprite)
    ...
    case SPR_CLIP:
      if (special->flags & (uint_64_t)(0x0000000000020000))
        {
    ...
    case SPR_MGUN:
      if (!P_GiveWeapon (player, wp_chaingun, (special->flags&(uint_64_t)(0x0000000000020000))!=0) )
        return;
    ...
    case SPR_SHOT:
      if (!P_GiveWeapon (player, wp_shotgun, (special->flags&(uint_64_t)(0x0000000000020000))!=0 ) )
        return;
    ...
    case SPR_SGN2:
      if (!P_GiveWeapon(player, wp_supershotgun, (special->flags&(uint_64_t)(0x0000000000020000))!=0))
        return;
  ...
  if (special->flags & (uint_64_t)(0x0000000000800000))
    player->itemcount++;
  ...
  if (!comp[comp_sound] || player == &players[displayplayer])
    S_StartSound (player->mo, sound | (0x8000));
}
...
static void P_KillMobj(mobj_t *source, mobj_t *target)
{
  ...
  target->flags &= ~((uint_64_t)(0x0000000000000004)|(uint_64_t)(0x0000000000004000)|(uint_64_t)(0x0000000001000000));

  if (target->type != MT_SKULL)
    target->flags &= ~(uint_64_t)(0x0000000000000200);

  target->flags |= (uint_64_t)(0x0000000000100000)|(uint_64_t)(0x0000000000000400);
  target->height >>= 2;

  if (!((target->flags ^ (uint_64_t)(0x0000000000400000)) & ((uint_64_t)0x0000000400000000ll | (uint_64_t)(0x0000000000400000))))
    totallive--;

  if (source && source->player)
    {

      if (target->flags & (uint_64_t)(0x0000000000400000))
  ...
  if (target->player)
    {
      ...
      target->flags &= ~(uint_64_t)(0x0000000000000002);
  ...
  mo->flags |= (uint_64_t)(0x0000000000020000);
}
# 745 "c:/devl/prboom3ds/src/p_inter.c"
void P_DamageMobj(mobj_t *target,mobj_t *inflictor, mobj_t *source, int damage)
{
  player_t *player;
  boolean justhit = 0;
  ...
  if (!(target->flags & ((uint_64_t)(0x0000000000000004) | (uint_64_t)0x0000000200000000ll)))
    return;

  if (target->health <= 0)
    return;

  if (target->flags & (uint_64_t)(0x0000000001000000))
  ...
  if (inflictor && !(target->flags & (uint_64_t)(0x0000000000001000)) &&
       ...
       source->player->readyweapon != wp_chainsaw))
    {
      ...
      fixed_t thrust = damage*((1<<16)>>3)*100/target->info->mass;
      ...
      if (target->intflags & MIF_FALLING && target->gear >= (6 +16))
        target->gear = 0;
    }
  ...
  {
    thinker_t *cap = &thinkerclasscap[target->flags & (uint_64_t)0x0000000400000000ll ?
  ...
  if (P_Random (pr_painchance) < target->info->painchance &&
      !(target->flags & (uint_64_t)(0x0000000001000000))) {
    if ((compatibility_level>=mbf_compatibility))
      justhit = 1;
    else
      target->flags |= (uint_64_t)(0x0000000000000040);
  ...
  if (source && source != target && source->type != MT_VILE &&
      ...
      ((source->flags ^ target->flags) & (uint_64_t)0x0000000400000000ll ||
       monster_infighting ||
    ...
    (!(compatibility_level>=mbf_compatibility) ?
     !target->lastenemy->player :
     !((target->flags ^ target->lastenemy->flags) & (uint_64_t)0x0000000400000000ll) &&
  ...
  if (justhit && (target->target == source || !target->target ||
      !(target->flags & target->target->flags & (uint_64_t)0x0000000400000000ll)))
    target->flags |= (uint_64_t)(0x0000000000000040);
}

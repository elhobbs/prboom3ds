# 1 "c:/devl/prboom3ds/src/r_things.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/r_things.c"
# 34 "c:/devl/prboom3ds/src/r_things.c"
...
# 35 "c:/devl/prboom3ds/src/r_things.c" 2
...
# 36 "c:/devl/prboom3ds/src/r_things.c" 2
...
# 37 "c:/devl/prboom3ds/src/r_things.c" 2
...
# 40 "c:/devl/prboom3ds/src/r_things.c" 2
...
# 41 "c:/devl/prboom3ds/src/r_things.c" 2
...
# 42 "c:/devl/prboom3ds/src/r_things.c" 2
...
void V_FreeScreens();
# 43 "c:/devl/prboom3ds/src/r_things.c" 2
...
# 44 "c:/devl/prboom3ds/src/r_things.c" 2
...
# 64 "c:/devl/prboom3ds/src/r_things.c"
...
# 152 "c:/devl/prboom3ds/src/r_things.c"
static void R_InitSpriteDefs(const char * const * namelist)
{
  ...
  hash = Z_Malloc(sizeof(*hash)*numentries,PU_STATIC,0);
  ...
  for (i=0; (size_t)i<numentries; i++)
    {
      int j = ((unsigned)((lumpinfo[i+firstspritelump].name)[0]-((lumpinfo[i+firstspritelump].name)[1]*3-(lumpinfo[i+firstspritelump].name)[3]*2-(lumpinfo[i+firstspritelump].name)[2])*2)) % numentries;
  ...
  for (i=0 ; i<numsprites ; i++)
    {
      ...
      int j = hash[((unsigned)((spritename)[0]-((spritename)[1]*3-(spritename)[3]*2-(spritename)[2])*2)) % numentries].index;
...
# 318 "c:/devl/prboom3ds/src/r_things.c"
int *mfloorclip;
int *mceilingclip;
fixed_t spryscale;
...
)
{
  ...
  for (i=0; i<column->numPosts; i++) {
      ...
      dcvars->yl = (topscreen+(1<<16)-1)>>16;
      dcvars->yh = (bottomscreen-1)>>16;
      ...
      if (dcvars->yl <= dcvars->yh && dcvars->yh < viewheight)
        {
          ...
          dcvars->texturemid = basetexturemid - (post->topdelta<<16);
...
static void R_DrawVisSprite(vissprite_t *vis, int x1, int x2)
{
  ...
  if (!dcvars.colormap)
    ...
    if (vis->mobjflags & (uint_64_t)(0x000000000c000000))
      {
        colfunc = R_GetDrawColumnFunc(RDC_PIPELINE_TRANSLATED, filter, filterz);
        dcvars.translation = translationtables - 256 +
          ((vis->mobjflags & (uint_64_t)(0x000000000c000000)) >> (26 -8) );
      }
    else
      if (vis->mobjflags & (uint_64_t)(0x0000000040000000) && general_translucency)
        {
  ...
  dcvars.iscale = FixedDiv ((1<<16), vis->scale);
...
static void R_ProjectSprite (mobj_t* thing, int lightlevel)
{
  ...
  if (sprframe->rotate)
    {
      ...
      unsigned rot = (ang-thing->angle+(unsigned)(0x20000000/2)*9)>>29;
...

  {
    ...
    if (flip) {
      tx -= (patch->width - patch->leftoffset) << 16;
    } else {
      tx -= patch->leftoffset << 16;
    }
  ...
  vis = R_NewVisSprite ();
# 615 "c:/devl/prboom3ds/src/r_things.c"
  ...
  if (thing->flags & (uint_64_t)(0x0000000000040000))
      vis->colormap = ((void *)0);
...
static void R_DrawPSprite (pspdef_t *psp, int lightlevel)
{
  ...
  avis.isplayersprite = 1;
# 709 "c:/devl/prboom3ds/src/r_things.c"
...
# 813 "c:/devl/prboom3ds/src/r_things.c"
}
...
# 855 "c:/devl/prboom3ds/src/r_things.c"
      ...
      if (num_vissprite_ptrs < num_vissprite*2)
        {
          ...
          vissprite_ptrs = Z_Malloc((num_vissprite_ptrs = num_vissprite_alloc*2) * sizeof *vissprite_ptrs,PU_STATIC,0)
                                                           ;
        }
...
# 945 "c:/devl/prboom3ds/src/r_things.c"
  for (ds=ds_p ; ds-- > drawsegs ; )
    {
      ...
      if (ds->silhouette&1 && spr->gz < ds->bsilheight)
        for (x=r1 ; x<=r2 ; x++)
          if (clipbot[x] == -2)
            clipbot[x] = ds->sprbottomclip[x];

      if (ds->silhouette&2 && spr->gzt > ds->tsilheight)
        for (x=r1 ; x<=r2 ; x++)
          if (cliptop[x] == -2)
...
# 1071 "c:/devl/prboom3ds/src/r_things.c"
  for (ds=ds_p ; ds-- > drawsegs ; )
    if (ds->maskedtexturecol)

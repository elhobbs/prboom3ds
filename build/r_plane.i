# 1 "c:/devl/prboom3ds/src/r_plane.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/r_plane.c"
# 51 "c:/devl/prboom3ds/src/r_plane.c"
...
# 52 "c:/devl/prboom3ds/src/r_plane.c" 2
...
# 54 "c:/devl/prboom3ds/src/r_plane.c" 2
...
# 55 "c:/devl/prboom3ds/src/r_plane.c" 2
...
# 56 "c:/devl/prboom3ds/src/r_plane.c" 2
...
# 57 "c:/devl/prboom3ds/src/r_plane.c" 2
...
# 58 "c:/devl/prboom3ds/src/r_plane.c" 2
...
# 59 "c:/devl/prboom3ds/src/r_plane.c" 2
...
# 60 "c:/devl/prboom3ds/src/r_plane.c" 2
...
void V_FreeScreens();
# 61 "c:/devl/prboom3ds/src/r_plane.c" 2
...
# 62 "c:/devl/prboom3ds/src/r_plane.c" 2



static visplane_t *visplanes[128];
...
int floorclip[400], ceilingclip[400];



static int spanstart[240];
...
static fixed_t cachedheight[240];
static fixed_t cacheddistance[240];
static fixed_t cachedxstep[240];
static fixed_t cachedystep[240];
static fixed_t xoffs,yoffs;

fixed_t yslope[240], distscale[400];
...
# 131 "c:/devl/prboom3ds/src/r_plane.c"
  ...
  if (!(dsvars->colormap = fixedcolormap))
    {
      ...
      dsvars->nextcolormap = planezlight[index+1 >= 128 ? 128 -1 : index+1];
    }
  else
   {
      dsvars->z = 0;
...
void R_ClearPlanes(void)
{
  ...
  for (i=0;i<128;i++)
    for (*freehead = visplanes[i], visplanes[i] = ((void *)0); *freehead; )
...
visplane_t *R_DupPlane(const visplane_t *pl, int start, int stop)
{
      unsigned hash = ((unsigned)((pl->picnum)*3+(pl->lightlevel)+(pl->height)*7) & (128 -1));
                        ...
                        fixed_t xoffs, fixed_t yoffs)
{
  ...
  if (picnum == skyflatnum || picnum & (0x80000000))
    height = lightlevel = 0;


  hash = ((unsigned)((picnum)*3+(lightlevel)+(height)*7) & (128 -1));
...
static void R_DoDrawPlane(visplane_t *pl)
{
  ...
  if (pl->minx <= pl->maxx) {
    if (pl->picnum == skyflatnum || pl->picnum & (0x80000000)) {
      int texture;
      ...
      if (pl->picnum & (0x80000000))
      {

        const line_t *l = &lines[pl->picnum & ~(0x80000000)];
        ...
        dcvars.texturemid = s->rowoffset - 28*(1<<16);
      ...
      dcvars.texheight = textureheight[skytexture]>>16;

      dcvars.iscale = (1<<16)*200/viewheight;
      ...
      tex_patch = R_CacheTextureCompositePatchNum(texture);
        ...
        for (x = pl->minx; (dcvars.x = x) <= pl->maxx; x++)
          if ((dcvars.yl = pl->top[x]) != -1 && dcvars.yl <= (dcvars.yh = pl->bottom[x]))
            {
              dcvars.source = R_GetTextureColumn(tex_patch, ((an + xtoviewangle[x])^flip) >> 22);
              dcvars.prevsource = R_GetTextureColumn(tex_patch, ((an + xtoviewangle[x-1])^flip) >> 22);
              dcvars.nextsource = R_GetTextureColumn(tex_patch, ((an + xtoviewangle[x+1])^flip) >> 22);
              colfunc(&dcvars);
            }

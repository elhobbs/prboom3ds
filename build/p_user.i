# 1 "c:/devl/prboom3ds/src/p_user.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/p_user.c"
# 36 "c:/devl/prboom3ds/src/p_user.c"
...
# 37 "c:/devl/prboom3ds/src/p_user.c" 2
...
# 38 "c:/devl/prboom3ds/src/p_user.c" 2
...
# 39 "c:/devl/prboom3ds/src/p_user.c" 2
...
# 40 "c:/devl/prboom3ds/src/p_user.c" 2
...
# 41 "c:/devl/prboom3ds/src/p_user.c" 2
# 1 "c:/devl/prboom3ds/src/p_user.h" 1
# 41 "c:/devl/prboom3ds/src/p_user.h"
void P_PlayerThink(player_t *player);
void P_CalcHeight(player_t *player);
void P_DeathThink(player_t *player);
void P_MovePlayer(player_t *player);
void P_Thrust(player_t *player, angle_t angle, fixed_t move);
# 42 "c:/devl/prboom3ds/src/p_user.c" 2
...
# 43 "c:/devl/prboom3ds/src/p_user.c" 2
...
# 44 "c:/devl/prboom3ds/src/p_user.c" 2
# 57 "c:/devl/prboom3ds/src/p_user.c"
boolean onground;
...
# 83 "c:/devl/prboom3ds/src/p_user.c"
static void P_Bob(player_t *player, angle_t angle, fixed_t move)
{
  ...
  player->momx += FixedMul(move,finecosine[angle >>= 19]);
...
void P_CalcHeight (player_t* player)
{
  int angle;
  fixed_t bob;
# 117 "c:/devl/prboom3ds/src/p_user.c"
  if (!(compatibility_level < boom_compatibility_compatibility) && !player_bobbing)
    player->bob = 0;
  else
  {
    fixed_t x = (compatibility_level>=mbf_compatibility) ? player->momx : player->mo->momx;
    fixed_t y = (compatibility_level>=mbf_compatibility) ? player->momy : player->mo->momy;
    ...
    if (compatibility_level >= boom_202_compatibility &&
        ...
        player->mo->friction > 0xE800)
    {
      if (player->bob > (0x100000>>2))
        player->bob = 0x100000>>2;
    }
    else
...
# 199 "c:/devl/prboom3ds/src/p_user.c"
void P_MovePlayer (player_t* player)
{
...
# 219 "c:/devl/prboom3ds/src/p_user.c"
  if ((!(compatibility_level < boom_compatibility_compatibility) && !(compatibility_level>=mbf_compatibility)) || (cmd->forwardmove | cmd->sidemove))
    {
      if (onground || mo->flags & (uint_64_t)0x0000000200000000ll)
      {
        ...
        int bobfactor =
          friction < 0xE800 ? movefactor : 2048;
        ...
        if (cmd->sidemove)
        {
          P_Bob(player,mo->angle-0x40000000,cmd->sidemove*bobfactor);
          P_Thrust(player,mo->angle-0x40000000,cmd->sidemove*movefactor);
        }
      }
...
# 257 "c:/devl/prboom3ds/src/p_user.c"
void P_DeathThink (player_t* player)
{
  angle_t angle;
  angle_t delta;
...
void P_PlayerThink (player_t* player)
{
  ...
  if (player->cheats & CF_NOCLIP)
    player->mo->flags |= (uint_64_t)(0x0000000000001000);
  else
    player->mo->flags &= ~(uint_64_t)(0x0000000000001000);
  ...
  cmd = &player->cmd;
  if (player->mo->flags & (uint_64_t)(0x0000000000000080))
    {
    cmd->angleturn = 0;
  ...
  player->fixedcolormap = player->powers[pw_invulnerability] > 4*32 ||
    player->powers[pw_invulnerability] & 8 ? 32 :

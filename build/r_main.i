# 1 "c:/devl/prboom3ds/src/r_main.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/r_main.c"
# 40 "c:/devl/prboom3ds/src/r_main.c"
...
# 41 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 42 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 43 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 44 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 45 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 46 "c:/devl/prboom3ds/src/r_main.c" 2
# 1 "c:/devl/prboom3ds/src/r_bsp.h" 1
# 41 "c:/devl/prboom3ds/src/r_bsp.h"
...
# 47 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 49 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 50 "c:/devl/prboom3ds/src/r_main.c" 2
...
void V_FreeScreens();
# 51 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 52 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 53 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 54 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 55 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 56 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 57 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 58 "c:/devl/prboom3ds/src/r_main.c" 2
...
# 59 "c:/devl/prboom3ds/src/r_main.c" 2
# 67 "c:/devl/prboom3ds/src/r_main.c"
int viewangleoffset;
int validcount = 1;
...
angle_t xtoviewangle[400 +1];




int numcolormaps;
const lighttable_t *(*c_zlight)[16][128];
const lighttable_t *(*zlight)[128];
...
int extralight;
# 125 "c:/devl/prboom3ds/src/r_main.c"
__attribute__((pure)) int R_PointOnSide(fixed_t x, fixed_t y, const node_t *node)
{
  if (!node->dx)
  ...
  if ((node->dy ^ node->dx ^ x ^ y) < 0)
    return (node->dy ^ x) < 0;
  return FixedMul(y, node->dx>>16) >= FixedMul(node->dy>>16, x);
}
...
__attribute__((pure)) int R_PointOnSegSide(fixed_t x, fixed_t y, const seg_t *line)
{
  ...
  if ((ldy ^ ldx ^ x ^ y) < 0)
    return (ldy ^ x) < 0;
  return FixedMul(y, ldx>>16) >= FixedMul(ldy>>16, x);
}
# 179 "c:/devl/prboom3ds/src/r_main.c"
...
# 180 "c:/devl/prboom3ds/src/r_main.c" 2
...
angle_t R_PointToAngle(fixed_t x, fixed_t y)
{
  ...
  if (
      ...
      (x < 2147483647/4 && x > -2147483647/4 && y < 2147483647/4 && y > -2147483647/4)
     )
  {
    ...
    x >= 0 ?
      y >= 0 ?
        (x > y) ? tantoangle[SlopeDiv(y,x)] :
                0x40000000 -1-tantoangle[SlopeDiv(x,y)] :
        x > (y = -y) ? 0-tantoangle[SlopeDiv(y,x)] :
                       0xc0000000 +tantoangle[SlopeDiv(x,y)] :
      y >= 0 ? (x = -x) > y ? 0x80000000 -1-tantoangle[SlopeDiv(y,x)] :
                            0x40000000 + tantoangle[SlopeDiv(x,y)] :
        (x = -x) > (y = -y) ? 0x80000000 +tantoangle[ SlopeDiv(y,x)] :
                              0xc0000000 -1-tantoangle[SlopeDiv(x,y)] :
    0;
  }
  ...
  if (oldx != x || oldy != y)
  {
    oldx = x;
    oldy = y;
    oldresult = (int)(atan2(y, x) * 0x80000000/3.14159265358979323846);
  }
  return oldresult;
}
...
static void R_InitTextureMapping (void)
{
  register int i,x;
  fixed_t focallength;
# 255 "c:/devl/prboom3ds/src/r_main.c"
...
static void R_InitLightTables (void)
{
  ...
  for (i=0; i< 16; i++)
    {
      int j, startmap = ((16 -1-i)*2)*32/16;
      for (j=0; j<128; j++)
        {


          int scale = FixedDiv ((320/2*(1<<16)), (j+1)<<20);
          int t, level = startmap - (scale >>= 12)/2;
...
# 346 "c:/devl/prboom3ds/src/r_main.c"
...
void R_ExecuteSetViewSize (void)
{
  ...
  if (setblocks == 11)
    ...
    {
      scaledviewwidth = SCREENWIDTH;
      viewheight = SCREENHEIGHT-(32*SCREENHEIGHT/200);
    }
  else
    {

      scaledviewwidth = setblocks*SCREENWIDTH/10;
      viewheight = (setblocks*(SCREENHEIGHT-(32*SCREENHEIGHT/200))/10) & ~7;
    }

  viewwidth = scaledviewwidth;

  viewheightfrac = viewheight<<16;
  ...
  projectiony = ((SCREENHEIGHT * centerx * 320) / 200) / SCREENWIDTH * (1<<16);
  ...
  pspritescale = (1<<16)*viewwidth / 400;
  pspriteiscale = (1<<16)*400 / viewwidth;

  pspriteyscale = (((SCREENHEIGHT*viewwidth) / SCREENWIDTH) << 16) / 240;
  ...
  for (i=0 ; i<viewheight ; i++)
    {
      fixed_t dy = D_abs(((i-viewheight/2)<<16)+(1<<16)/2);
  ...
  for (i=0 ; i<viewwidth ; i++)
    {
      fixed_t cosadj = D_abs(finecosine[xtoviewangle[i]>>19]);
...
subsector_t *R_PointInSubsector(fixed_t x, fixed_t y)
{
  ...
  return &subsectors[nodenum & ~0x80000000];
}
...
{
# 555 "c:/devl/prboom3ds/src/r_main.c"
  static int keeptime[10], showtime;
  int now = I_GetTime();
  ...
  if (now - showtime > 35) {
    ...
    (35*10)/(now - keeptime[0]), rendered_segs,
    rendered_visplanes, rendered_vissprites);
    showtime = now;
  }
  memmove(keeptime, keeptime+1, sizeof(keeptime[0]) * (10 -1));
  keeptime[10 -1] = now;
...
void R_RenderPlayerView_ (player_t* player)
{
...
void R_RenderPlayerView(player_t* player)
{
 ...
 angle_t ang = (slideamt*(0x40000000/18));
 ...
 vsin = finesine[(viewangle - 0x40000000) >> 19];
 vcos = finecosine[(viewangle - 0x40000000) >> 19];

# 1 "c:/devl/prboom3ds/src/p_pspr.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/p_pspr.c"
# 35 "c:/devl/prboom3ds/src/p_pspr.c"
...
# 36 "c:/devl/prboom3ds/src/p_pspr.c" 2
...
# 37 "c:/devl/prboom3ds/src/p_pspr.c" 2
...
# 38 "c:/devl/prboom3ds/src/p_pspr.c" 2
...
# 39 "c:/devl/prboom3ds/src/p_pspr.c" 2
...
void A_RandomJump();
void A_LineEffect();
# 41 "c:/devl/prboom3ds/src/p_pspr.c" 2
...
# 42 "c:/devl/prboom3ds/src/p_pspr.c" 2
...
extern int idmusnum;
# 43 "c:/devl/prboom3ds/src/p_pspr.c" 2
...
# 44 "c:/devl/prboom3ds/src/p_pspr.c" 2
...
# 45 "c:/devl/prboom3ds/src/p_pspr.c" 2
...
# 46 "c:/devl/prboom3ds/src/p_pspr.c" 2
# 54 "c:/devl/prboom3ds/src/p_pspr.c"
...
# 120 "c:/devl/prboom3ds/src/p_pspr.c"
static void P_BringUpWeapon(player_t *player)
{
  ...
  player->psprites[ps_weapon].sy =
    (compatibility_level>=mbf_compatibility) ? ((1<<16)*128)+(1<<16)*2 : ((1<<16)*128);
...
{
  int *prefer = weapon_preferences[(compatibility_level < boom_compatibility_compatibility)!=0];
    ...
    switch (*prefer++)
      ...
      case 7:
        if (player->weaponowned[wp_bfg] && gamemode != shareware &&
            player->ammo[am_cell] >= ((compatibility_level < boom_compatibility_compatibility) ? 41 : 40))
          newweapon = wp_bfg;
        break;
      case 8:
      ...
      case 9:
        if (player->weaponowned[wp_supershotgun] && gamemode == commercial &&
            player->ammo[am_shell] >= ((compatibility_level < boom_compatibility_compatibility) ? 3 : 2))
...
# 236 "c:/devl/prboom3ds/src/p_pspr.c"
...
# 276 "c:/devl/prboom3ds/src/p_pspr.c"
boolean P_CheckAmmo(player_t *player)
{
  if (P_GetAmmoLevel(player, player->readyweapon) > 0)
    return 1;
# 288 "c:/devl/prboom3ds/src/p_pspr.c"
...
# 333 "c:/devl/prboom3ds/src/p_pspr.c"
void A_WeaponReady(player_t *player, pspdef_t *psp)
{
  ...
  {
    int angle = (128*leveltime) & (8192 -1);
    psp->sx = (1<<16) + FixedMul(player->bob, finecosine[angle]);
    angle &= 8192/2-1;
    psp->sy = ((1<<16)*32) + FixedMul(player->bob, finesine[angle]);
  }
}
...
# 481 "c:/devl/prboom3ds/src/p_pspr.c"
static void A_FireSomething(player_t* player,int adder)
{
  ...
  if (!(player->mo->flags & (uint_64_t)(0x0000000000001000)))
    if (!(compatibility_level<=boom_compatibility_compatibility) && weapon_recoil)
      P_Thrust(player,
               0x80000000 +player->mo->angle,
...
# 513 "c:/devl/prboom3ds/src/p_pspr.c"
void A_Punch(player_t *player, pspdef_t *psp)
{
  ...
  if (!(compatibility_level>=mbf_compatibility) ||
      (slope = P_AimLineAttack(player->mo, angle, (64*(1<<16)), (uint_64_t)0x0000000400000000ll),
       !linetarget))
    slope = P_AimLineAttack(player->mo, angle, (64*(1<<16)), 0);
...
void A_Saw(player_t *player, pspdef_t *psp)
{
  ...
  if (!(compatibility_level>=mbf_compatibility) ||
      (slope = P_AimLineAttack(player->mo, angle, (64*(1<<16))+1, (uint_64_t)0x0000000400000000ll),
       !linetarget))
    slope = P_AimLineAttack(player->mo, angle, (64*(1<<16))+1, 0);
  ...
  player->mo->flags |= (uint_64_t)(0x0000000000000080);
  R_SmoothPlaying_Reset(player);
}
...
static void P_BulletSlope(mobj_t *mo)
{
  ...
  do
    {
      bulletslope = P_AimLineAttack(mo, an, 16*64*(1<<16), mask);
      if (!linetarget)
  bulletslope = P_AimLineAttack(mo, an += 1<<26, 16*64*(1<<16), mask);
      if (!linetarget)
  bulletslope = P_AimLineAttack(mo, an -= 2<<26, 16*64*(1<<16), mask);
    }
  ...
  for (i=0; i<20; i++)
    {
      ...
      P_LineAttack(player->mo, angle, (32*64*(1<<16)), bulletslope +
...
void A_BFGSpray(mobj_t *mo)
{
  ...
  for (i=0 ; i<40 ; i++)
    {
      int j, damage;
      angle_t an = mo->angle - 0x40000000/2 + 0x40000000/40*i;




      if (!(compatibility_level>=mbf_compatibility) ||
         (P_AimLineAttack(mo->target, an, 16*64*(1<<16), (uint_64_t)0x0000000400000000ll),
         !linetarget))
        P_AimLineAttack(mo->target, an, 16*64*(1<<16), 0);

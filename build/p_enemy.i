# 1 "c:/devl/prboom3ds/src/p_enemy.c"
# 1 "c:\\devl\\prboom3ds\\build//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "c:/devl/prboom3ds/src/p_enemy.c"
# 36 "c:/devl/prboom3ds/src/p_enemy.c"
...
# 37 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 38 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 39 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 40 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 41 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 42 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 43 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
extern int idmusnum;
# 44 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 45 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 46 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 47 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
void A_RandomJump();
void A_LineEffect();
# 48 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 49 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 50 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 51 "c:/devl/prboom3ds/src/p_enemy.c" 2
...
# 85 "c:/devl/prboom3ds/src/p_enemy.c"
...
static boolean P_CheckMeleeRange(mobj_t *actor)
{
  ...
  return
    pl && !(actor->flags & pl->flags & (uint_64_t)0x0000000400000000ll) &&
    (P_AproxDistance(pl->x-actor->x, pl->y-actor->y) <
     (64*(1<<16)) - 20*(1<<16) + pl->info->radius) &&
...
{
  return actor->flags & (uint_64_t)0x0000000400000000ll && actor->target &&
    ...
    !((linetarget->flags ^ actor->flags) & (uint_64_t)0x0000000400000000ll);
}
...
static boolean P_CheckMissileRange(mobj_t *actor)
{
  ...
  if (actor->flags & (uint_64_t)(0x0000000000000040))
    {
      actor->flags &= ~(uint_64_t)(0x0000000000000040);
      ...
      return
  !(actor->flags & (uint_64_t)0x0000000400000000ll) ||
  (actor->target->health > 0 &&
   (!(actor->target->flags & (uint_64_t)0x0000000400000000ll) ||
    (actor->target->player ?
     monster_infighting || P_Random(pr_defect) >128 :
     !(actor->target->flags & (uint_64_t)(0x0000000000000040)) && P_Random(pr_defect) >128)));
    }




  if (actor->flags & actor->target->flags & (uint_64_t)0x0000000400000000ll)
    return 0;
...
# 253 "c:/devl/prboom3ds/src/p_enemy.c"
...
# 290 "c:/devl/prboom3ds/src/p_enemy.c"
...
static fixed_t xspeed[8] = {(1<<16),47000,0,-47000,-(1<<16),-47000,0,47000};
static fixed_t yspeed[8] = {0,47000,(1<<16),47000,0,-47000,-(1<<16),-47000};
...
static boolean P_Move(mobj_t *actor, boolean dropoff)
{
  ...
  if (actor->movedir == DI_NODIR)
    return 0;
# 333 "c:/devl/prboom3ds/src/p_enemy.c"
  if (monster_friction)
  ...
  if (try_ok && friction > 0xE800)
    {
      actor->x = origx;
      actor->y = origy;
      movefactor *= (1<<16) / 2048 / 4;
  ...
  if (!try_ok)
    {
      ...
      if (actor->flags & (uint_64_t)(0x0000000000004000) && floatok)
        {
          if (actor->z < tmfloorz)
            actor->z += ((1<<16)*4);
          else
          ...
          actor->flags |= (uint_64_t)(0x0000000000200000);
...
# 398 "c:/devl/prboom3ds/src/p_enemy.c"
  ...
  if (!(actor->flags & (uint_64_t)(0x0000000000004000)) &&
...
# 450 "c:/devl/prboom3ds/src/p_enemy.c"
  if ((actor->type == MT_DOGS || (actor->type == (HelperThing-1) && actor->flags&(uint_64_t)0x0000000400000000ll))
      && target && dog_jumping &&
      !((target->flags ^ actor->flags) & (uint_64_t)0x0000000400000000ll) &&
      P_AproxDistance(actor->x - target->x,
          actor->y - target->y) < (1<<16)*144 &&
  ...
  if (
      (on_lift && P_Random(pr_stayonlift) < 230 &&
       !P_IsOnLift(actor))
      ||
      (monster_avoid_hazards && !under_damage &&
...
# 488 "c:/devl/prboom3ds/src/p_enemy.c"
...
# 505 "c:/devl/prboom3ds/src/p_enemy.c"
static void P_DoNewChaseDir(mobj_t *actor, fixed_t deltax, fixed_t deltay)
{
  ...
  xdir =
    deltax > 10*(1<<16) ? DI_EAST :
    deltax < -10*(1<<16) ? DI_WEST : DI_NODIR;

  ydir =
    deltay < -10*(1<<16) ? DI_SOUTH :
    deltay > 10*(1<<16) ? DI_NORTH : DI_NODIR;
...
# 571 "c:/devl/prboom3ds/src/p_enemy.c"
...
static boolean PIT_AvoidDropoff(line_t *line)
{
  if (line->backsector &&
      ...
      P_BoxOnLineSide(tmbbox, line) == -1)
    {
      ...
      if (back == floorz && front < floorz - (1<<16)*24)
  angle = R_PointToAngle2(0,0,line->dx,line->dy);
      else
  if (front == floorz && back < floorz - (1<<16)*24)
    ...
    return 1;
      ...
      dropoff_deltax -= finesine[angle >> 19]*32;
      dropoff_deltay += finecosine[angle >> 19]*32;
    }
  return 1;
}
...
{
  int yh=((tmbbox[BOXTOP] = actor->y+actor->radius)-bmaporgy)>>(16 +7);
  int yl=((tmbbox[BOXBOTTOM]= actor->y-actor->radius)-bmaporgy)>>(16 +7);
  int xh=((tmbbox[BOXRIGHT] = actor->x+actor->radius)-bmaporgx)>>(16 +7);
  int xl=((tmbbox[BOXLEFT] = actor->x-actor->radius)-bmaporgx)>>(16 +7);
  int bx, by;
...
static void P_NewChaseDir(mobj_t *actor)
{
  ...
  if ((compatibility_level>=mbf_compatibility)) {
    if (actor->floorz - actor->dropoffz > (1<<16)*24 &&
  actor->z <= actor->floorz &&
  !(actor->flags & ((uint_64_t)(0x0000000000000400)|(uint_64_t)(0x0000000000004000))) &&
  !comp[comp_dropoff] &&
  ...
  if (actor->flags & target->flags & (uint_64_t)0x0000000400000000ll &&
      distfriend << 16 > dist &&
      !P_IsOnLift(target) && !P_IsUnderDamage(actor))
  {
    deltax = -deltax, deltay = -deltay;
  } else
    if (target->health > 0 && (actor->flags ^ target->flags) & (uint_64_t)0x0000000400000000ll)
      {
        if (monster_backing &&
      actor->info->missilestate && actor->type != MT_SKULL &&
      ((!target->info->missilestate && dist < (64*(1<<16))*2) ||
       (target->player && dist < (64*(1<<16))*3 &&
...
static boolean P_IsVisible(mobj_t *actor, mobj_t *mo, boolean allaround)
{
  if (!allaround)
    ...
    P_AproxDistance(mo->x-actor->x, mo->y-actor->y) > (64*(1<<16)))
  return 0;
    }
  return P_CheckSight(actor, mo);
}
# 730 "c:/devl/prboom3ds/src/p_enemy.c"
...
static boolean PIT_FindTarget(mobj_t *mo)
{
  ...
  if (!((mo->flags ^ actor->flags) & (uint_64_t)0x0000000400000000ll &&
  mo->health > 0 && (mo->flags & (uint_64_t)(0x0000000000400000) || mo->type == MT_SKULL)))
    return 1;
  ...
  (targ->flags ^ mo->flags) & (uint_64_t)0x0000000400000000ll &&
  ...
  {
    thinker_t *cap = &thinkerclasscap[mo->flags & (uint_64_t)0x0000000400000000ll ?
...
static boolean P_LookForPlayers(mobj_t *actor, boolean allaround)
{
  ...
  if (actor->flags & (uint_64_t)0x0000000400000000ll)
    {
      int anyone;
  ...
  stop = (actor->lastlook-1)&(4 -1);

  c = 0;

  stopc = !(compatibility_level>=mbf_compatibility) &&
    !(compatibility_level < boom_compatibility_compatibility) && monsters_remember ?
    4 : 2;

  for (;; actor->lastlook = (actor->lastlook+1)&(4 -1))
    {
  ...
  continue;
      ...
      if (c++ == stopc || actor->lastlook == stop)
      {
        ...
        if (!(compatibility_level>=mbf_compatibility) && !(compatibility_level < boom_compatibility_compatibility) && monsters_remember)
        {
          if (actor->lastenemy && actor->lastenemy->health > 0)
          {
            actor->target = actor->lastenemy;
            actor->lastenemy = ((void *)0);
            return 1;
          }
        }
...
# 877 "c:/devl/prboom3ds/src/p_enemy.c"
static boolean P_LookForMonsters(mobj_t *actor, boolean allaround)
{
  ...
  if (actor->lastenemy && actor->lastenemy->health > 0 && monsters_remember &&
      !(actor->lastenemy->flags & actor->flags & (uint_64_t)0x0000000400000000ll))
    {
      P_SetTarget(&actor->target, actor->lastenemy);
      P_SetTarget(&actor->lastenemy, ((void *)0));
      return 1;
    }
  ...
  cap = &thinkerclasscap[actor->flags & (uint_64_t)0x0000000400000000ll ? th_enemies : th_friends];



  if (cap->cnext != cap)
    {
      int x = (actor->x - bmaporgx)>>(16 +7);
      int y = (actor->y - bmaporgy)>>(16 +7);
      int d;
...
{
  return actor->flags & (uint_64_t)0x0000000400000000ll ?
...
static boolean P_HelpFriend(mobj_t *actor)
{
  ...
  cap = &thinkerclasscap[actor->flags & (uint_64_t)0x0000000400000000ll ? th_friends : th_enemies];
  ...
  if (P_Random(pr_helpfriend) < 180)
    break;
      ...
      if (((mobj_t *) th)->flags & (uint_64_t)(0x0000000000000040) &&
    ((mobj_t *) th)->target &&
...
void A_KeenDie(mobj_t* mo)
{
  ...
  for (th = thinkerclasscap[th_all].next ; th != &thinkerclasscap[th_all] ; th=th->next)
...
# 1042 "c:/devl/prboom3ds/src/p_enemy.c"
void A_Look(mobj_t *actor)
{
  ...
  if (!(actor->flags & (uint_64_t)0x0000000400000000ll && P_LookForTargets(actor, 0)) &&
      !((targ = actor->subsector->sector->soundtarget) &&
  targ->flags & (uint_64_t)(0x0000000000000004) &&
  (P_SetTarget(&actor->target, targ),
   !(actor->flags & (uint_64_t)(0x0000000000000020)) || P_CheckSight(actor, targ))) &&
      (actor->flags & (uint_64_t)0x0000000400000000ll || !P_LookForTargets(actor, 0)))
    return;
...
# 1099 "c:/devl/prboom3ds/src/p_enemy.c"
...
void A_Chase(mobj_t *actor)
{
  ...
  if (!actor->target || !(actor->target->flags&(uint_64_t)(0x0000000000000004)))
    {
      if (!P_LookForTargets(actor,1))
  P_SetMobjState(actor, actor->info->spawnstate);
      return;
    }


  if (actor->flags & (uint_64_t)(0x0000000000000080))
    {
      actor->flags &= ~(uint_64_t)(0x0000000000000080);
  ...
  if (!(actor->target && actor->target->health > 0 &&
        ((comp[comp_pursuit] && !netgame) ||
         (((actor->target->flags ^ actor->flags) & (uint_64_t)0x0000000400000000ll ||
     (!(actor->flags & (uint_64_t)0x0000000400000000ll) && monster_infighting)) &&
    P_CheckSight(actor, actor->target))))
      && P_LookForTargets(actor, 1))
        return;
  ...
  if (!actor->info->missilestate && actor->flags & (uint_64_t)0x0000000400000000ll) {
    if (actor->flags & (uint_64_t)(0x0000000000000040))
      actor->flags &= ~(uint_64_t)(0x0000000000000040);
    else if (P_LookForPlayers(actor, 1))
      return;
  }
    }
  }
...
void A_FaceTarget(mobj_t *actor)
{
  if (!actor->target)
    return;
  actor->flags &= ~(uint_64_t)(0x0000000000000020);
  actor->angle = R_PointToAngle2(actor->x, actor->y,
                                 actor->target->x, actor->target->y);
  if (actor->target->flags & (uint_64_t)(0x0000000000040000))
    {
...
void A_PosAttack(mobj_t *actor)
{
  ...
  A_FaceTarget(actor);
  angle = actor->angle;
  slope = P_AimLineAttack(actor, angle, (32*64*(1<<16)), 0);
...
void A_SPosAttack(mobj_t* actor)
{
  ...
  A_FaceTarget(actor);
  bangle = actor->angle;
  slope = P_AimLineAttack(actor, bangle, (32*64*(1<<16)), 0);
  for (i=0; i<3; i++)
    {
...
void A_CPosRefire(mobj_t *actor)
{
  ...
  if (P_Random(pr_cposrefire) < 40) {
    if (actor->target && actor->flags & actor->target->flags & (uint_64_t)0x0000000400000000ll)
      goto stop;
    else
      return;
  }
...
void A_SpidRefire(mobj_t* actor)
{
  ...
  if (!actor->target || actor->target->health <= 0
      || actor->flags & actor->target->flags & (uint_64_t)0x0000000400000000ll
      || !P_CheckSight(actor, actor->target))
    stop: P_SetMobjState(actor, actor->info->seestate);
}
...
void A_Tracer(mobj_t *actor)
{
  ...
  mobj_t *dest;
  mobj_t *th;
# 1481 "c:/devl/prboom3ds/src/p_enemy.c"
...
static boolean PIT_VileCheck(mobj_t *thing)
{
  ...
  if (!(thing->flags & (uint_64_t)(0x0000000000100000)) )
    return 1;
  ...
  if (D_abs(thing->x-viletryx) > maxdist || D_abs(thing->y-viletryy) > maxdist)
    return 1;
# 1602 "c:/devl/prboom3ds/src/p_enemy.c"
    corpsehit = thing;
    ...
    if (comp[comp_vile])
    ...
    else
      {
        ...
        corpsehit->flags |= (uint_64_t)(0x0000000000000002);
        check = P_CheckPosition(corpsehit,corpsehit->x,corpsehit->y);
  ...
  if (actor->movedir != DI_NODIR)
    {
      ...
      corpsehit->flags =
        (info->flags & ~(uint_64_t)0x0000000400000000ll) | (actor->flags & (uint_64_t)0x0000000400000000ll);

    if (!((corpsehit->flags ^ (uint_64_t)(0x0000000000400000)) & ((uint_64_t)0x0000000400000000ll | (uint_64_t)(0x0000000000400000))))
      totallive++;

                  corpsehit->health = info->spawnhealth;
      P_SetTarget(&corpsehit->target, ((void *)0));

      if ((compatibility_level>=mbf_compatibility))
        {
          P_SetTarget(&corpsehit->lastenemy, ((void *)0));
          corpsehit->flags &= ~(uint_64_t)(0x0000000000000040);
        }
...
# 1830 "c:/devl/prboom3ds/src/p_enemy.c"
...
# 1908 "c:/devl/prboom3ds/src/p_enemy.c"
void A_SkullAttack(mobj_t *actor)
{
  ...
  dest = actor->target;
  actor->flags |= (uint_64_t)(0x0000000001000000);
...
static void A_PainShootSkull(mobj_t *actor, angle_t angle)
{
  ...
  if (comp[comp_pain])
    {
      ...
      int count = 0;
      thinker_t *currentthinker = ((void *)0);
      while ((currentthinker = P_NextThinker(currentthinker,th_all)) != ((void *)0))
  ...
  newmobj->flags = (newmobj->flags & ~(uint_64_t)0x0000000400000000ll) | (actor->flags & (uint_64_t)0x0000000400000000ll);
...
void A_Fall(mobj_t *actor)
{
  ...
  actor->flags &= ~(uint_64_t)(0x0000000000000002);
}
...
void P_SpawnBrainTargets(void)
{
  ...
  for (thinker = thinkerclasscap[th_all].next ;
       thinker != &thinkerclasscap[th_all] ;
        ...
        if (m->type == MT_BOSSTARGET )
          {
            if (numbraintargets >= numbraintargets_alloc)
              braintargets = Z_Realloc(braintargets,(numbraintargets_alloc = numbraintargets_alloc ? numbraintargets_alloc*2 : 32) *sizeof *braintargets,PU_STATIC,0)
...
void A_BrainScream(mobj_t *mo)
{
  ...
  for (x=mo->x - 196*(1<<16) ; x< mo->x + 320*(1<<16) ; x+= (1<<16)*8)
    {
      int y = mo->y - 320*(1<<16);
      int z = 128 + P_Random(pr_brainscream)*2*(1<<16);
...
void A_BrainExplode(mobj_t *mo)
{
  ...
  int y = mo->y;
  int z = 128 + P_Random(pr_brainexp)*2*(1<<16);
  mobj_t *th = P_SpawnMobj(x,y,z, MT_ROCKET);
...
void A_BrainSpit(mobj_t *mo)
{
  ...
  newmobj->flags = (newmobj->flags & ~(uint_64_t)0x0000000400000000ll) | (mo->flags & (uint_64_t)0x0000000400000000ll);
...
void A_SpawnFly(mobj_t *mo)
{
  ...
  if (P_LookForTargets(newmobj,1))
    P_SetMobjState(newmobj, newmobj->info->seestate);


  P_TeleportMove(newmobj, newmobj->x, newmobj->y, 1);
...
{
  P_DamageMobj(actor, ((void *)0), ((void *)0), actor->health);
}
...
void A_Mushroom(mobj_t *actor)
{
  ...
  target.x += i << 16;
  target.y += j << 16;
  target.z += P_AproxDistance(i,j) << (16 +2);
  mo = P_SpawnMissile(actor, &target, MT_FATSHOT);
  mo->momx >>= 1;
  mo->momy >>= 1;
  mo->momz >>= 1;
  mo->flags &= ~(uint_64_t)(0x0000000000000200);
      }
}
# 2539 "c:/devl/prboom3ds/src/p_enemy.c"
void A_Spawn(mobj_t *mo)
{
  if (mo->state->misc1)
    {

      P_SpawnMobj(mo->x, mo->y, (mo->state->misc2 << 16) + mo->z,
      mo->state->misc1 - 1);
...
{
  S_StartSound(mo->state->misc2 ? ((void *)0) : mo, mo->state->misc1);
}
